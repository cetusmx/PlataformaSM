...
const SubirInventario = ({ onUploadSuccess }) => {
    ...
    const handleAuditorDropdownChange = (e) => {
        ...
        if (selectedAuditorName) {
            const selectedAuditor = auditores.find(auditor => auditor.Nombre === selectedAuditorName);

            if (selectedAuditor && !auditoresSeleccionados.some(a => a.Nombre === selectedAuditor.Nombre)) {
                setAuditoresSeleccionados(prevSelected => [...prevSelected, selectedAuditor]);
            }
    ...
    const handleRemoveAuditorTag = (auditorToRemove) => {
        setAuditoresSeleccionados(prevSelected =>
            prevSelected.filter(auditor => auditor.Nombre !== auditorToRemove.Nombre)
        );
    };
                    ...
                    <div className="form-group">
                        ...
                        >
                            ...
                            {loadingAuditores ? (
                                ...
                                auditores
                                    .filter(auditor => !auditoresSeleccionados.some(selected => selected.Nombre === auditor.Nombre))
                                    .map((auditor, index) => (
                        ...
                        <div className="selected-tags-container auditor-tags-container"> {/* Nueva clase para el scroll de los tags */}
                            {auditoresSeleccionados.map((auditor, index) => (
                                <span key={index} className="selected-tag">
                                    {auditor.Nombre}
                                    <button onClick={() => handleRemoveAuditorTag(auditor)}>x</button>
                                </span>
                            ))}
                            {auditoresSeleccionados.length === 0 && !loadingAuditores && (
                                <p className="no-data-message-small">No hay auditores seleccionados.</p>
                            )}
                        </div>
                    </div>
